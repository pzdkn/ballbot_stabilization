...
for t=0:dt:max_t
  ...
  u = [-Kx'*yz ;...
   ...
   K_ = [K1; K2; K3];
   K_real = (A*diag([1,-1,1])*K_);
   x_ = [theta(1); dtheta(1); theta(2); dtheta(2); theta(3); dtheta(3);
       phi(1); dphi(1); phi(2); dphi(2)];
   u_ = -K_real*x_;
   setMotorTorques(clientInfo, u_);
         
    % Real K for state space ordering of 2d 
%     K1 = [Kx', 0, 0, 0, 0, 0, 0];
%     K2 = [0, 0, 0, 0, Ky', 0, 0];
